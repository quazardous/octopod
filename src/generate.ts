...
export interface EdgeSettings {
  ...
  /** The folder of the API's unix socket, on the host. */
  socketDir: string;
...
/**
 * The console relay: GET and HEAD to the API's socket, anything else refused — the console
 * reads, it does not act. When the API is not running, a page that says how to start it.
 */
export function consoleNginxConfig(socketName: string): string {
  return `# Generated by octopod: the console relay, from the edge to the API's unix socket.
worker_processes 1;
pid /tmp/nginx.pid;
...
http {
  ...
  server {
    ...
    location @down {
      ...
      return 503 '<!doctype html><meta charset="utf-8"><title>octopod</title><body style="font:16px system-ui;margin:3rem"><h1>octopod is not serving</h1><p>The console needs the API: run <code>octopod serve</code>, or start its service (<code>systemctl --user start octopod</code>).</p>';
    }
  }
}
`;
  ...
  return {
    ...
    services: {
      ...
      console: {
        ...
        volumes: [`${settings.consoleConfig}:/etc/nginx/nginx.conf:ro`, `${settings.socketDir}:/run/octopod:ro`],
        labels: {
          ...digest,
          [EDGE_LABEL]: settings.instance,
          'traefik.enable': 'true',
